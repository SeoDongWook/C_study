    ...
    case WM_PAINT:
        {
			...
			irr::core::vector2df vNewOrigin(160, 120); // (0,0) 에서 중앙까지 가기위한 이동값
			grp.TranslateTransform(vNewOrigin.X, vNewOrigin.Y); // ******** 앞으로 자동으로 알아서 더하도록...!

			irr::core::vector2df vTest1(0, 0); // 값 안넣어도 0,0
			irr::core::vector2df vTest2(100, 100);

			// grp.DrawLine(&pen, vTest1.X, vTest1.Y, vTest2.X, vTest2.Y); // 원점잡기 전

			// vTest1 += vNewOrigin; // 중앙설정하기.
			// vTest2 += vNewOrigin;
			
			grp.DrawLine(&pen, vTest1.X, vTest1.Y, vTest2.X, vTest2.Y); // 원점잡은 후 선 그리기.

			grp.ResetTransform(); // TranslateTransform 끝내는 함수.
