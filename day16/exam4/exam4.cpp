...
//
//
REAL g_fScale = 1.0; // 방향키 증,감 값 선언.
LRESULT CALLBACK WndProc(HWND hWnd, UINT message, WPARAM wParam, LPARAM lParam)
{

    switch (message)
    {
	case WM_KEYDOWN: // 방향키를 이용해 실시간으로 원크기 바꾸기 위해서...
		switch (wParam)
		{
		case VK_UP:
			g_fScale += 0.1;
		...
		InvalidateRect(hWnd, NULL, TRUE); // 이거안하면 실시간 출력 안됨.
		break;
	//------------------------------------------------------------------//
    case WM_COMMAND:
        {
    ...
    case WM_PAINT:
        {
			...
			grp.ScaleTransform(g_fScale, g_fScale); // 2배로 키우기
