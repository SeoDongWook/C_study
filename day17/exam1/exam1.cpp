...
//
//
irr::core::vector2df g_vTestPt(50, 150);
    ...
    case WM_PAINT:
        {
			...
			irr::core::vector2df vStart(-50, -50);
			irr::core::vector2df vEnd(50, 150);

			grp.DrawLine(&pen, vStart.X, vStart.Y, vEnd.X, vEnd.Y);
			plusEngine::printf(&grp, vEnd.X, vEnd.Y, L"끝");

			irr::core::line2df line1(vStart.X, vStart.Y, vEnd.X, vEnd.Y);
			irr::f32 result = line1.getPointOrientation(g_vTestPt);

			grp.DrawRectangle(&pen, vStart.X-4, vStart.Y-4, 8., 8.);
			if (result > 0) {
				plusEngine::printf(&grp, g_vTestPt.X, g_vTestPt.Y - 14, L"오른쪽 %lf", result);
			}
			else {
				plusEngine::printf(&grp, g_vTestPt.X, g_vTestPt.Y - 14, L"왼쪽 %lf", result);
			}
